 ...
 * - Hybrid conversation history (Bot + Human)
...
export default function AdminSupportPortal() {
  ...
  // 3. Group flat messages into "Threads"
  const threads = useMemo(() => {
    if (!rawMessages) return [];
    
    const userGroups = new Map<string, any>();
    
    // Iterate in reverse (oldest to newest) or just use latest logic
  ...
  // 4. Logic for ACTIVE chat window
  ...
  if (isUserLoading || checkingAdmin) {
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-[#EEEEFF] space-y-4">
        ...
        <p className="text-sm font-bold text-[#1A1A2E] animate-pulse uppercase tracking-widest">Authorized Portal Syncing...</p>
      </div>
    );
  }
      ...
      <main className="flex-1 flex overflow-hidden">
        ...
        <section className="w-80 bg-white border-r border-[#E8E6FF] flex flex-col shadow-sm relative z-10">
          <header className="p-6 space-y-4 border-b border-[#E8E6FF]">
            <div>
              <h2 className="text-xl font-bold text-[#1A1A2E]">Priority Queue</h2>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar">
            {isMessagesLoading ? (
              ...
              <div className="p-10 text-center opacity-40 italic text-sm">No {filter} tickets found.</div>
            ) : (
              threads.map(thread => (
                <button 
                  key={thread.userId} 
                ...
                >
                  <div className="flex justify-between items-start">
                    <div className="flex items-center gap-2 overflow-hidden">
                      <div className="h-8 w-8 rounded-lg bg-[#EDE9FF] flex items-center justify-center shrink-0">
        ...
        <section className="flex-1 flex flex-col bg-[#F8F9FF] relative">
          {activeThread ? (
            <>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-[#22C55E] font-black uppercase tracking-widest mt-1.5 flex items-center gap-1.5">
                      <span className="h-2 w-2 rounded-full bg-[#22C55E] animate-pulse" /> Live Monitoring
                    </p>
                  </div>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-6" ref={scrollRef}>
                <AnimatePresence initial={false}>
                  {activeChatMessages.map((m, i) => (
                    ...
                    >
                      <div className={`max-w-[65%] ${m.sender === 'admin' ? 'items-end' : 'items-start'}`}>
                        <div className={`p-4 rounded-2xl text-sm shadow-sm transition-all ${
                          ...
                          <div className={`text-[9px] mt-2 font-black uppercase tracking-widest opacity-60 ${m.sender === 'admin' ? 'text-white' : 'text-[#64748B]'}`}>
              ...
              <div className="px-8 py-6 bg-white border-t border-[#E8E6FF] z-20 shadow-md">
                <div className="max-w-4xl mx-auto flex gap-4 items-center bg-[#F8F9FF] border border-[#E8E6FF] p-2 rounded-2xl">
                  <Input 
                    placeholder="Type response to worker..." 
                    value={replyText} 
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-10 opacity-40">
              <MessageSquare size={64} className="text-[#6C47FF] mb-4" />
              <h3 className="text-xl font-bold text-[#1A1A2E]">Select a conversation</h3>
              <p className="text-sm">Manage live worker escalations and parametric issues.</p>
            </div>
          )}
        </section>
      </main>
