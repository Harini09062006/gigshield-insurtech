  ...
  { 
    ...
    loading: () => (
      <div className="w-full h-full flex flex-col items-center justify-center bg-[#F5F3FF]">
        ...
        <p className="mt-4 text-[10px] font-bold text-[#6C47FF] uppercase tracking-widest animate-pulse">Initializing India Risk Core...</p>
      </div>
    )
  }
);
    ...
    <div className="h-screen w-full bg-[#EEEEFF] flex flex-col overflow-hidden font-body">
      ...
      <header className="px-6 py-3 flex items-center justify-between border-b border-[#E8E6FF] bg-white z-[2000] shadow-sm shrink-0">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <div className="h-8 w-8 bg-[#6C47FF] rounded-lg flex items-center justify-center shadow-sm">
      ...
      <main className="flex-1 relative overflow-hidden">
        ...
        <section className="absolute inset-0">
          ...
          <div className="absolute bottom-5 left-5 z-[1000] bg-white/95 backdrop-blur-md border border-[#E8E6FF] p-3 rounded-xl shadow-lg min-w-[160px]">
            <p className="text-[9px] font-black text-[#1A1A2E] uppercase tracking-widest mb-2 border-b border-[#E8E6FF] pb-1">Precipitation Legend</p>
            <div className="space-y-1.5">
              <div className="flex items-center gap-2"><div className="h-2 w-2 rounded-full bg-[#EF4444]" /><span className="text-[9px] font-bold text-[#1A1A2E]">Extreme (&gt;50mm)</span></div>
              <div className="flex items-center gap-2"><div className="h-2 w-2 rounded-full bg-[#F59E0B]" /><span className="text-[9px] font-bold text-[#1A1A2E]">High (&gt;30mm)</span></div>
              <div className="flex items-center gap-2"><div className="h-2 w-2 rounded-full bg-[#EAB308]" /><span className="text-[9px] font-bold text-[#1A1A2E]">Medium (&gt;10mm)</span></div>
              <div className="flex items-center gap-2"><div className="h-2 w-2 rounded-full bg-[#22C55E]" /><span className="text-[9px] font-bold text-[#1A1A2E]">Low (&gt;0mm)</span></div>
              <div className="flex items-center gap-2"><div className="h-2 w-2 rounded-full bg-[#64748B]" /><span className="text-[9px] font-bold text-[#1A1A2E]">Safe (0mm)</span></div>
            </div>
          </div>

          {/* Clean Filter Bar (Bottom Center) */}
            ...
            <div className="bg-white/95 backdrop-blur-md p-1.5 rounded-full border border-[#E8E6FF] shadow-lg flex gap-1 items-center">
              {['all', 'extreme', 'high', 'medium', 'low', 'safe'].map(r => (
                <button 
                  ...
                  className={`px-3 py-1 rounded-full text-[9px] font-black uppercase transition-all ${riskFilter === r ? 'bg-[#6C47FF] text-white shadow-sm' : 'hover:bg-[#F5F3FF] text-[#64748B]'}`}
                >
                  {r}
                </button>
              ))}
          ...
          <div className="absolute top-5 left-1/2 -translate-x-1/2 z-[1000] pointer-events-none">
            <div className="bg-white/90 backdrop-blur-md px-3 py-1 rounded-full border border-[#E8E6FF] shadow-md flex items-center gap-2">
              <RefreshCw className={`h-2.5 w-2.5 text-[#6C47FF] ${isLoading ? 'animate-spin' : ''}`} />
              <span className="text-[9px] font-bold text-[#64748B]">Updated: {lastUpdated.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
            </div>
          </div>
        </section>
      </main>
